...
-- Modules
local GradientCanvas = require(script:WaitForChild("FastCanvas")) -- Credits to BoatBomber (originally FastCanvas)
...
function CanvasDraw.new(Frame: GuiObject, Resolution: Vector2?, CanvasColour: Color3?)
	...
	if Resolution then
		if Resolution.X > CanvasResolutionLimit.X or Resolution.Y > CanvasResolutionLimit.Y then
			OutputWarn(
				"A canvas cannot be built with a resolution larger than "
					.. CanvasResolutionLimit.X
					.. " x "
					.. CanvasResolutionLimit.Y
					.. "."
			)
	...
	function Canvas:GetMousePoint(): Vector2?
		if RunService:IsClient() then
			...
			local GradientCanvasFrameSize = self.CurrentCanvasFrame.VPFCanvas.AbsoluteSize
			...
			if not SurfaceGui then
				...
				local RatioDifference = Vector2New(
					CanvasFrameSize.X / GradientCanvasFrameSize.X,
					CanvasFrameSize.Y / GradientCanvasFrameSize.Y
				) - Vector2New(1, 1)
				...
				if
					...
					and TransformedPoint.X <= self.CurrentResX
					and TransformedPoint.Y <= self.CurrentResY
				then
				...
				local GradientCanvasFrame = Frame:FindFirstChild("VPFCanvas")
				...
				if Part and GradientCanvasFrame then
					...
					if Result then
						...
						local ScreenPos = Vector2.new(math.abs(hitOffset.X), math.abs(hitOffset.Y))
			...
			OutputWarn(
				"Failed to get point from mouse (you cannot use this function on the server. Please call this function from a LocalScript)."
			)
		end
	end
	...
	function Canvas:CreateImageDataFromCanvas(PointA: Vector2, PointB: Vector2): {}
		...
		return {
			ImageColours = ColoursData,
			ImageAlphas = AlphasData,
			ImageResolution = Vector2New(ImageResolutionX, ImageResolutionY),
		}
	end

	function Canvas:DrawImageXY(
		ImageData,
		X: number?,
		Y: number?,
		ScaleX: number?,
	...
	function Canvas:FloodFill(Point: Vector2, Colour: Color3) -- Optimised by @Arevoir
		...
		while #queue > 0 do
			...
			if
				currentPointX > 0
				and currentPointY > 0
				and currentPointX <= canvasWidth
				and currentPointY <= canvasHeight
			then
	...
	function Canvas:DrawTriangleXY(
		X1: number,
		Y1: number,
		X2: number,
		Y2: number,
		...
		if Fill or typeof(Fill) == "nil" then
			...
			if CheckPoint(X1, Y1) or CheckPoint(X2, Y2) or CheckPoint(X3, Y3) then
				OutputWarn(
					"DrawTriangle (XY) Error: This drawing method doesn't have clipping (Points exceed bounds) Canceling..."
				)
				return
			end
	...
	function Canvas:DrawDistortedImageXY(X1, Y1, X2, Y2, X3, Y3, X4, Y4, ImageData, Brightness: number?)
		Canvas:DrawTexturedTriangleXY(X1, Y1, X2, Y2, X3, Y3, 0, 0, 0, 1, 1, 1, ImageData, Brightness)
	...
	function Canvas:DrawLine(PointA: Vector2, PointB: Vector2, Colour: Color3, Thickness: number?): {}
		...
		if not Thickness or Thickness < 1 then
			...
			local TrianglePointsA = Canvas:DrawTriangle(
				Vector2New(StartCornerX1, StartCornerY1),
				Vector2New(StartCornerX2, StartCornerY2),
				Vector2New(EndCornerX1, EndCornerY1),
				Colour
			)
			local TrianglePointsB = Canvas:DrawTriangle(
				Vector2New(StartCornerX2, StartCornerY2),
				Vector2New(EndCornerX1, EndCornerY1),
				Vector2New(EndCornerX2, EndCornerY2),
				Colour
			)
...
function CanvasDraw.GetImageData(SaveObject: Instance)
	...
	local ImageData =
		{ ImageColours = PixelColours, ImageAlphas = PixelAlphas, ImageResolution = SaveDataImageResolution }
...
function CanvasDraw.CreateSaveObject(ImageData, InstantCreate: boolean?): Folder
	if
		ImageData.ImageResolution.X > SaveObjectResolutionLimit.X
		and ImageData.ImageResolution.Y > SaveObjectResolutionLimit.Y
	then
		warn(
			[[Failed to create an image save object (ImageData too large). 
		Please try to keep the resolution of the image no higher than ']]
				.. SaveObjectResolutionLimit.X
				.. " x "
				.. SaveObjectResolutionLimit.Y
				.. "'."
		)
		return
	end
